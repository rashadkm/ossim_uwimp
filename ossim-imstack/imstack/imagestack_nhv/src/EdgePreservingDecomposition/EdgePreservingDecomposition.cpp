...
float *EdgePreservingDecomposition::CreateBlur(float *Source, float Scale, float EdgeStopping, unsigned int Iterates, float *Blur){
	...
	//Create the edge stopping function a, rotationally symmetric and just one instead of (ax, ay). Don't need Blur yet, so use its memory.
	float *a = Blur;
	unsigned int x, y, i;
	...
	for(y = 0; y != h1; y++){
		...
		for(x = 0; x != w1; x++){
			...
			//Apply power to the magnitude of the gradient to get the edge stopping function. Also, divide by mean of pixels in this square
			//to get logarithmic derivative. The extra factor of 2 cancels part of the 4 missing in the denominator, so multiply by 2.
			a[x + w*y] = Scale*powf(2.0f*sqrtf(gx*gx + gy*gy)/(rg[x + 1] + rg[x] + rg[x + w + 1] + rg[x + w] + eps) + eps, -EdgeStopping);
		}
	}
...
float *EdgePreservingDecomposition::CompressDynamicRange(float *Source, float Scale, float EdgeStopping, float Compression, float DetailBoost, unsigned int Iterates, float *Compressed){
	//Blur. Also setup mempry for Compressed (we can just use u since each element of u is used in one calculation).
	float *u = CreateBlur(Source, Scale, EdgeStopping, Iterates);
	if(Compressed == NULL) Compressed = u;

	//Apply an exponent to the blur and boost details. Details are boosted linearly and contrast decreased logarithmically.
	Compression = 1.0f - Compression;	//This way input compression goes from 0 (no effect) to 1 (full effect).
	for(unsigned int i = 0; i != n; i++)
		Compressed[i] = (Source[i] + DetailBoost*(Source[i] - u[i]))/(u[i] + 1e-7f)*powf(u[i], Compression);
...
void ModifyCIELabLuminance(float *NewL, unsigned short *L, unsigned short *a, unsigned short *b, unsigned int npx, float SaturationControl){
	while(npx-- != 0){		//Loop over all pixels.
		//Using Mantiuk's suggested color replacement, except that SaturationControl isn't here adapted to any model.
		float ao = (((float)a[npx]/L[npx] - 1.0f)*SaturationControl + 1.0f)*NewL[npx];
		float bo = (((float)b[npx]/L[npx] - 1.0f)*SaturationControl + 1.0f)*NewL[npx];

		//What follows is pixel format dependent (above isn't).
		long c, Range = 65535;

		c = (long)((float)Range*NewL[npx]);
		L[npx] = (unsigned short)(c < 0 ? 0 : (c > Range ? Range : c));

		c = (long)((float)Range*ao);
		a[npx] = (unsigned short)(c < 0 ? 0 : (c > Range ? Range : c));

		c = (long)((float)Range*bo);
		b[npx] = (unsigned short)(c < 0 ? 0 : (c > Range ? Range : c));
	}
}
