...
class Expression {
    ...
    Funct1  -> sin | cos | tan | log | lag10 | abs | mean | sum | max | min | stddev | variance | skew | kurtosis
    Funct2  -> covariance
  ...
  public:
    ...
    struct Funct_tanh : public Unary {
        Funct_tanh(Node *arg) : Unary(arg) {}
        float eval(State *state) {return tanhf(arg->eval(state));}
    };
    ...
    struct Funct_lag10 : public Unary {
        Funct_lag10(Node *arg) : Unary(arg) {}
        float eval(State *state) {return log10f(arg->eval(state));}
    };
