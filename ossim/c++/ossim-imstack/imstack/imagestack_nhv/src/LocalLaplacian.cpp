...
// fast method when only working with luminance
// passed image must be in Lab  Luv  etc space
void myLocalLaplacian::help() {
    pprintf("-locallaplacian1 modifies contrast at various scales. It is similar to"
            " the clarity slider in Photoshop. This operation is an implementation"
            " of Fast and Robust Pyramid-based Image Processing by Aubry et al,"
            " which is an acceleration of Local Laplacian Filters by Paris et"
            " al.\n\n"
...
void myLocalLaplacian::parse(vector<string> args) {
    assert(args.size() == 2, "-locallaplacian takes two arguments");
...
//#pragma omp parallel for
    ...
    out = apply(out, readFloat(args[0]), readFloat(args[1]));
...
#if TEST
				float val = _max - out(x, y, t)[0];
				im(x, y, t)[0] += val;
//				im(x, y, t)[0] = 1.0 - im(x, y, t)[0];
#else
				im(x, y, t)[0] = out(x, y, t)[0];
#endif
	    	}
		}
    }
...
Image myLocalLaplacian::apply(Window im, float alpha, float beta) {
    const int K = 8, J = 8;

    // Compute a discretized set of K intensities that span the values in the image
    Stats s = Stats(im);
	float stats_divisor = 1.0 / (s.maximum() - s.minimum());
    ...
    for (int i = 0; i < K; i++) {
        ...
        for (int t = 0; t < im.frames; t++) {
            for (int y = 0; y < im.height; y++) {
                for (int x = 0; x < im.width; x++) {
                    float luminance = p(x, y, t)[0];;
                    float v = luminance - target[i];
                    float adjustment = alpha * v * fastexp(sigma*v*v);
                    //printf("At level %d, %f -> %f\n", i, luminance, luminance + adjustment);
                    p(x, y, t)[0] += adjustment;
                }
            }
        }
        processed[i] = p;
        ...
        for (int t = 0; t < imPyramid[j].frames; t++) {
            for (int y = 0; y < imPyramid[j].height; y++) {
                for (int x = 0; x < imPyramid[j].width; x++) {
                    ...
                    float alpha = luminance - K0;
                    float modified =
                         alpha * pyramid[K1][j](x, y, t)[0] +
                          (1 - alpha) * pyramid[K0][j](x, y, t)[0];
                    imLPyramid[j](x, y, t)[0] =
