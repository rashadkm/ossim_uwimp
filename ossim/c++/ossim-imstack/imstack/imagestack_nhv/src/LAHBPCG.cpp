...
//			AD(x,y,t)[0] = sx(x,y,t)[0] + add_x + w(x,y,t)[0] + sy(x,y,t)[0] + add_y;
			AD(x,y,t)[0] = sx(x,y,t)[0] + add_x + 1.0 + sy(x,y,t)[0] + add_y;
...
//		a2 = sx(x,y,t)[0] + sx(x+1,y,t)[0] + w(x,y,t)[0]; // a_(ij)x_(ij) + w_(ij)*x_(ij)
		a2 = sx(x,y,t)[0] + sx(x+1,y,t)[0] + 1; // a_(ij)x_(ij) + w_(ij)*x_(ij)
		a3 = -sx(x+1,y,t)[0]; // AW
		for (int c = 0; c < im.channels; c++) {
...
void PCG::solve(Window guess, int max_iter, float tol) {
    ...
    printf("\tPCG::solve() initial max_iter: %d,  epsilon %f,  error: %f\n", max_iter, epsilon, delta);
	...
	resNorm = dot(r,r);
	if (resNorm < epsilon) {
		printf("\niteration  resNorm < epsilon    %d, target %f, error %f", iter, epsilon, resNorm);
	    break;
	}
	...
	dr = s;
	Add::apply(dr,dr_tmp);
		printf("\r\titeration %d, target %f, error %f                          ", iter, epsilon, delta); fflush(stdout);
    }
		printf("\n\tPCG::solve() iteration %d, epsilon %f, error %f\n", iter-1, epsilon, delta);
}
...
Image LAHBPCG::apply(Window d, Window gx, Window gy, Window w, Window sx, Window sy, int max_iter, float tol)
{
    ...
    Image out(d.width, d.height, d.frames, d.channels);
      // LAHBPCG::apply(L, zeros, zeros, w, Lx, Ly, 200, tolerance);
      //apply(Window d, Window gx, Window gy, Window w, Window sx, Window sy, int max_iter, float tol)
