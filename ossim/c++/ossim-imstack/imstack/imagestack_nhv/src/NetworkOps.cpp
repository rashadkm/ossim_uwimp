...
/*
void nhvReceive::help() {
    printf("\n-receive sets up a TCP server and listens for an imagestack command sting string\n"
           "-send). The sole optional argument is the port to listen on. It defaults\n"
           "to 5678.\n\n"
           "Usage: ImageStack -receive 5243 -save image.jpg\n\n"
           "       ImageStack -loop --receive --scale 2 --send somewhereelse\n\n");
...
void nhvReceive::parse(vector<string> args) {
    assert(args.size() < 1, "-receive <args string> \n");
    ipush(apply(args);
}


Image nhvReceive::apply(int port, vector<string>args) {
    // create and bind the server if it hasn't already been created
    if (!servers[port]) {
        servers[port] = new TCPServer(port);
    }
