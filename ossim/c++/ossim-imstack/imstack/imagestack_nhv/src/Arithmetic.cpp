...
void SinMap::help() {
    printf("\n-sinmap applies a cheesy sin curve as a tone map\n\n"
	   "Usage: ImageStack -load a.tga -sinmap -save out.tga.\n");
}

void SinMap::parse(vector<string> args) {
    assert(args.size() == 0, "-sinmap takes no arguments\n");
    apply(stack(0));
}

void SinMap::apply(Window a) {
	#ifndef M_PI_F
	#define M_PI_F 3.14159265f
	#define M_PI_F_2 (3.14159265f / 2.0f)
	#endif
	//float scale = 0.4f;
	// compute mean
	for (int t = 0; t < a.frames; t++) {
		...
		// set hist mean to scale
		float mean[3] = { 0.0f, 0.0f, 0.0f };
		for (int c = 0; c < a.channels; c++) {
			mean[c] = sum[c] / (a.height * a.width);
			//mean[c] = scale / mean[c];
		}
#endif // 0
		// apply a sin wave stretch
		for (int y = 0; y < a.height; y++) {
	    		for (int x = 0; x < a.width; x++) {
				for (int c = 0; c < a.channels; c++) {
					float val =  (a(x,y,t)[c]);
					//float val = a(x,y,t)[c] *scale/mean[c];
					val = val < 0.0f ? 0.0f : val > 1.0f ? 1.0f : val;
					                        //(0.5  *(sin ( ([2]*pi)         -  pi/2)            +1))
		    			a(x, y, t)[c] = 0.5f * (sinf( (val*M_PI_F) - M_PI_F_2 ) + 1.0f); 
				}
	    		}
		}
	}
...
void myLog::help() {
    printf("\n-log takes the natural log of the current image.  log(val+1)\n\n"
	   "Usage: ImageStack -load a.tga -log -load b.tga -log -add -exp -save product.tga.\n");
}
    
void myLog::parse(vector<string> args) {
    assert(args.size() == 0, "-log takes no arguments\n");
    apply(stack(0));
}

void myLog::apply(Window a) {
    for (int t = 0; t < a.frames; t++) {
	for (int y = 0; y < a.height; y++) {
	    for (int x = 0; x < a.width; x++) {
		for (int c = 0; c < a.channels; c++) {
		    a(x, y, t)[c] = logf(a(x, y, t)[c] + 1.0f);
		}
	    }
	}
    }
...
void Log10::help() {
    printf("\n-log10 takes the natural log of the current image + 1.\n\n"
	   "Usage: ImageStack -load a.tga -log110 -load b.tga -lag10 -add -exp -save product.tga.\n");
}
    
void Log10::parse(vector<string> args) {
    assert(args.size() == 0, "-log10 takes no arguments\n");
    apply(stack(0));
}

void Log10::apply(Window a) {
    for (int t = 0; t < a.frames; t++) {
	for (int y = 0; y < a.height; y++) {
	    for (int x = 0; x < a.width; x++) {
		for (int c = 0; c < a.channels; c++) {
		    a(x, y, t)[c] = log10f(a(x, y, t)[c]+1);
		}
	    }
	}
    }
...
void myExp::help() {
    printf("\nWith no arguments -exp calculates e to the current image. exp(val)-1\n\n"
	   "Usage: ImageStack -load a.tga -log -load b.tga -log -add -exp -save product.tga.\n");
}
    
void myExp::parse(vector<string> args) {
    if (args.size() == 0) apply(stack(0));
    else if (args.size() == 1) apply(stack(0), readFloat(args[1]));
    else panic("-exp takes zero or one arguments\n");
}

void myExp::apply(Window a, float base) {
    for (int t = 0; t < a.frames; t++) {
	for (int y = 0; y < a.height; y++) {
	    for (int x = 0; x < a.width; x++) {
		for (int c = 0; c < a.channels; c++) {
		    a(x, y, t)[c] = powf(base, a(x, y, t)[c] - 1.0f);
		}
	    }
	}
    }
...
void Stretch::help() {
    printf("\n-stretch restricts the image to be between Min and 1\n"
	   "by rescaling it.\n\n"
	   "Usage: ImageStack -load a.exr -normalize -save a.tga\n\n");
}
    
void Stretch::parse(vector<string> args) {
    assert(args.size() == 0, "-stretch takes no arguments\n");
    apply(stack(0));
}


void Stretch::apply(Window a) {
    float minValue = a(0, 0)[0];
    float maxValue = a(0, 0)[0];
